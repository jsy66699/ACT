...
**卡住的是盒子，不是 `|z|` 的大小。** 20 个实例的盒中心、600 个神经元：
...
`|z|` 并不是一律很大——8.6% 的神经元离 0 不到 1.0。真正的限制是 ε=0.015 的盒子
能把预激活推动的幅度：按「一堵墙落在盒子一阶梯度预算之内才算可达」来数，
**只有约 4% 的神经元够得着 0 这堵墙**。

三段划分在 `z = −τ, +τ` 各切一刀，给这些神经元一个够得着的边界。同一口径下，实例 #0：
sigmoid 24/600 → **54/600**，tanh 20/600 → **40/600**。绝对数逐实例波动较大
（前 5 个实例 17–52 对 39–91），稳定的是**比值 1.75–2.4 倍**。
