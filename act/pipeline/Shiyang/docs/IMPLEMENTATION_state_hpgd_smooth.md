...
上，这一位仍然在 0 处切开，而在验证盒里它**几乎恒定**，准入永远说「见过」。

**卡住的是盒子，不是 `|z|` 的大小**（2026-09-14 重测，20 个实例的盒中心，600 个神经元）：

| | Sigmoid 6×100 | Tanh 6×100 |
|---|---|---|
| `\|z\|` 中位数 | 4.96 | 6.46 |
| `\|z\|` 最小值 | **0.0015** | 0.0016 |
| 逐**层**中位数的范围 | 3.29 – 22.67 | 3.88 – 23.04 |
| `\|z\| < 1` 的占比 | 8.6 % | 6.9 % |
| `z > 0` 的占比 | 33.0 % | 47.8 % |

所以**不是**「所有 `|z|` 都 ≥ 3.5」——那个 3.5–22 其实是**逐层中位数**的范围，
早期一版 docstring 把它当成 `|z|` 本身的范围写了，是错的。真正的原因是 ε=0.015 的盒子
能把预激活推动的幅度有限：按「一堵墙落在盒子一阶梯度预算之内才算可达」来数，
**只有约 4% 的神经元够得着 0 这堵墙**。

三段划分在 `z = −τ, +τ` 各切一刀，给这些神经元一个够得着的边界。同一口径下（**实例 #0**）：
sigmoid 24/600 → **54/600**，tanh 20/600 → **40/600**。这是单个实例不是平均——
前 5 个实例的绝对数在 17–52（sign）对 39–91（±τ）之间跳，但**比值稳定在 1.75–2.4 倍**。
